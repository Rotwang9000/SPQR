...
function computeVersionFromDimension(dimension) {
    const provisional = Math.floor((dimension - 17) / 4);
    return Math.max(1, Math.min(40, provisional));
}
function buildFunctionMask(dimension) {
    const mask = new Set();
    const key = (x, y) => y * 2048 + x; // sufficient range
    ...
    if (centers) {
        for (let i = 0; i < centers.length; i++) {
            for (let j = 0; j < centers.length; j++) {
                ...
                if (overlapsTL || overlapsTR || overlapsBL)
                    continue;
    ...
    return {
        dimension,
        has(x, y) { return mask.has(key(x, y)); },
    };
}
function dataMaskPredicate(maskId) {
...
function decodeFormatFromBits(bits) {
    let best = null;
    let bestDist = 16;
...
function readFormatInfo(matrix) {
    ...
    for (let x = 0; x <= 8; x++)
        if (x !== 6) {
            bitsTL = (bitsTL << 1) | (matrix.get(x, 8) ? 1 : 0);
        }
    // Then (8,7..0) excluding (8,6)
    for (let y = 7; y >= 0; y--)
        if (y !== 6) {
            bitsTL = (bitsTL << 1) | (matrix.get(8, y) ? 1 : 0);
        }
    ...
    let bitsTR = 0;
    // (dim-1 .. dim-8, 8)
    for (let x = dim - 1; x >= dim - 8; x--) {
        bitsTR = (bitsTR << 1) | (matrix.get(x, 8) ? 1 : 0);
    }
    // (8, dim-7 .. dim-1)
    for (let y = dim - 7; y <= dim - 1; y++) {
        bitsTR = (bitsTR << 1) | (matrix.get(8, y) ? 1 : 0);
    }
    const decTR = decodeFormatFromBits(bitsTR);
    return decTR;
}
function readCodewords(matrix, version, dataMaskId) {
    ...
    const debug = false;
    if (debug)
        console.log(`Reading codewords: v${version} mask${dataMaskId} dim${dimension}`);
    for (let col = dimension - 1; col > 0; col -= 2) {
        if (col === 6)
            col--; // skip timing column
        if (debug)
            console.log(`Column pair: ${col}`);
        for (let i = 0; i < dimension; i++) {
            ...
            for (let dx = 0; dx < 2; dx++) {
                ...
                if (!fnMask.has(x, y)) {
                    ...
                    bits++;
                    if (debug && bits <= 32)
                        console.log(`  (${x},${y}) raw=${matrix.get(x, y)} mask=${masked} final=${bit} bits=${bits} byte=${byte.toString(2).padStart(8, '0')}`);
                    if (bits === 8) {
                        ...
                        if (debug)
                            console.log(`    -> codeword[${codewords.length - 1}]: ${byte} -> ${reversed} (0x${reversed.toString(16)})`);
                        byte = 0;
                        bits = 0;
                    }
                }
    ...
    if (debug)
        console.log(`Total codewords: ${codewords.length}, first 8: [${codewords.slice(0, 8).map(b => '0x' + b.toString(16)).join(', ')}]`);
    return codewords;
}
function getDataByteCount(version, ec) {
...
function decodeByteStream(bytes, versionNumber) {
    ...
    try {
        ...
        while (available() >= 4) {
            ...
            if (mode === 0x4) { // Byte mode only
                ...
                try {
                    text += decodeURIComponent(out.map(b => `%${('0' + b.toString(16)).slice(-2)}`).join(''));
                }
                catch {
                    text += str;
                }
...
export function decodeMatrixGuessMask(matrix) {
    ...
    for (let rot = 0; rot < 4; rot++) {
        for (const mir of [false, true]) {
            ...
            if (fmt) {
                ...
                if (text)
                    return { text, mask: fmt.mask, version };
            }
...
export function decodeMatrixWithParams(matrix, params) {
    assert(matrix.width === matrix.height, 'Matrix must be square');
    const dimension = matrix.width;
