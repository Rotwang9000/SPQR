...
export function splitPayload(payload, splits) {
    assert(splits >= 1 && splits <= 8, 'splits must be 1..8');
    const parts = [];
    const size = Math.ceil(payload.length / splits);
    for (let i = 0; i < splits; i++)
        parts.push(payload.slice(i * size, (i + 1) * size));
    return parts;
}
async function encodeSquareMatrix(text, ecc) {
    ...
    for (let y = 0; y < size; y++) {
        ...
        for (let x = 0; x < size; x++)
            row.push(segments.modules.get(x, y) ? 1 : 0);
        qr.push(row);
    }
    return { qr, size };
}
async function encodeSquareMatrixWithVersion(text, version, ecc) {
...
async function encodeRmqrMatrix(text) {
    const Rmqr = rmqrModule.rmqr;
    const correction = (rmqrModule.correction ?? { auto: 0, medium: 1, high: 2 }).high;
    const inst = new Rmqr();
    const { qr, width, height } = await inst.generate(text, { correction });
    return { qr, width, height };
}
export async function generateColourQr(payload, opts = {}) {
    ...
    const composition = opts.composition ?? 'discrete';
    const ecc = opts.eccLevel ?? 'M';
    ...
    if (mode === 'square') {
        ...
        matrices = await Promise.all(textsForLayers.map(async (text) => {
    ...
    for (const m of matrices)
        assert(m.width === width && m.height === height, 'all layers must match dimensions');
            ...
            for (let x = 0; x < width; x++) {
                ...
                if (colours.length === 4) {
                    ...
                    switch (layerCode) {
                        case 0:
                            fill = colours[0];
                            break; // 00 → white (base=0, red=0)
                        case 1:
                            fill = colours[1];
                            break; // 01 → red (base=0, red=1)
                        case 2:
                            fill = colours[3];
                            break; // 10 → black (base=1, red=0)  
                        case 3:
                            fill = colours[2];
                            break; // 11 → green (base=1, red=1)
                    }
                }
                else {
                    ...
                    switch (layerCode) {
                        case 0:
                            fill = colours[0] ?? '#ffffff';
                            break; // 000: white
                        case 1:
                            fill = colours[1] ?? '#ff0000';
                            break; // 001: red
                        case 2:
                            fill = colours[2] ?? '#00ff00';
                            break; // 010: green  
                        case 3:
                            fill = colours[3] ?? '#ffff00';
                            break; // 011: yellow
                        case 4:
                            fill = colours[4] ?? '#000000';
                            break; // 100: black
                        case 5:
                            fill = colours[5] ?? '#ff00ff';
                            break; // 101: magenta
                        case 6:
                            fill = colours[6] ?? '#00ffff';
                            break; // 110: cyan
                        case 7:
                            fill = colours[7] ?? '#0000ff';
                            break; // 111: blue
                    }
                }
                const px = (x + margin) * moduleSize;
    ...
    if (opts.addKey !== false && (opts.finderKeyStyle ?? 'finderFill') === 'finderFill') {
        ...
        const drawAt = (fx, fy, cornerIndex) => {
            ...
            if (is8Color) {
                ...
                if (cornerIndex === 0)
                    ...
                    fill = black; // bl (spare finder) must be black
