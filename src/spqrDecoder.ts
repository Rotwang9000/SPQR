...
import { decodeMatrixGuessMask, matrixFromModules, BitMatrix } from './decoderCore.js';
...
function classifyColour(r: number, g: number, b: number) {
...
function enforceFindersAndTiming(mods: boolean[][]) {
	const n = mods.length;
...
function majorityFilter(mods: boolean[][]) {
...
function matrixFromMods(mods: boolean[][]): BitMatrix {
	return matrixFromModules(mods);
}

function estimateGrid(data: Uint8ClampedArray, width: number, height: number): GridInfo | null {
	...
	let best: GridInfo | null = null; let bestScore = -Infinity;
	for (const modules of candidates) {
		const margin = 4;
		const modulePx = qrW / (modules + 2 * margin);
		if (modulePx < 3 || modulePx > 50) continue;
		...
		if (score > bestScore) { bestScore = score; best = { modules, margin, modulePx, originX, originY }; }
	}
...
function scoreFinders(data: Uint8ClampedArray, width: number, height: number, modules: number, margin: number, modulePx: number, originX: number, originY: number): number {
...
export async function decodeSPQRIntegrated(inputPath: string): Promise<SPQRDecode> {
	let image = sharp(inputPath);
	let svgMeta: { modulesW?: number; modulesH?: number; margin?: number } | null = null;
	if (inputPath.toLowerCase().endsWith('.svg')) {
		...
		if (!Number.isFinite(svgMeta.modulesW) || !Number.isFinite(svgMeta.modulesH) || !Number.isFinite(svgMeta.margin)) svgMeta = null;
	}
	...
	const baseMods: boolean[][] = Array.from({ length: modules }, () => Array<boolean>(modules).fill(false));
	const redMods: boolean[][] = Array.from({ length: modules }, () => Array<boolean>(modules).fill(false));
			...
			for (let dy = -step; dy <= step; dy += step) {
				for (let dx = -step; dx <= step; dx += step) {
					...
					if (sx >= 0 && sx < width && sy >= 0 && sy < height) {
                        ...
                        rs += rgba[i]; gs += rgba[i + 1]; bs += rgba[i + 2]; c++;
					}
				}
			}
			const r = c ? rs / c : 255; const g = c ? gs / c : 255; const b = c ? bs / c : 255;
	...
	const decodeLayer = (mods: boolean[][], layerName: string): string | null => {
	...
	if (base && red) combined = base + red;
	else if (base) combined = base;
	else if (red) combined = red;
