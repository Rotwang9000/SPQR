...
export interface ColorMapping {
    white: number;    // 00 - no data
    red: number;      // 01 - red layer only
    green: number;    // 10 - green layer only  
    black: number;    // 11 - both layers (or base for 4-color)
}
...
export interface ColorBitMatrix {
    width: number;
    height: number;
    get: (x: number, y: number) => number; // returns color code 0-3
}

export function classifyPixelColor(r: number, g: number, b: number, colorScheme: 'bwrg' | 'cmyrgb' = 'bwrg'): number {
...
export function createColorBitMatrix(imageData: ImageData, qrLocation?: any): ColorBitMatrix {
...
export function decodeColorQR(colorMatrix: ColorBitMatrix): SPQRDecodeResult {
    ...
    try {
        ...
        if (baseResult) layers.push(baseResult);
    } catch (e) {
        console.log('Base layer decode failed:', (e as Error).message);
    }
    
    try {
        const redResult = decodeBinaryMatrix(redLayer);
        if (redResult) layers.push(redResult);
    } catch (e) {
        console.log('Red layer decode failed:', (e as Error).message);
    }
...
function createBinaryFromColors(colorMatrix: ColorBitMatrix, darkColors: number[]): boolean[][] {
...
function decodeBinaryMatrix(binary: boolean[][]): string | null {
...
export async function decodeSPQRImage(imageData: ImageData): Promise<SPQRDecodeResult> {
    try {
