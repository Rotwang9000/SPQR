...
import { decodeImageToText, decodeSvgMultiLayer, decodeRasterTwoLayer } from './decoder.js';
	...
	for (let i = 0; i < rest.length; i++) {
		...
		if (token.startsWith('--')) {
			...
			if (key === 'colours') {
				...
				const values: string[] = [];
				let j = i + 1;
				while (j < rest.length && !rest[j].startsWith('--')) { values.push(rest[j]); j++; }
				if (values.length > 0) { args[key] = values.join(' '); i = j - 1; }
				else { args[key] = true; }
			} else {
				const next = rest[i + 1];
...
function normaliseColoursArg(arg?: string | boolean): string[] | undefined {
	if (!arg || typeof arg === 'boolean') return undefined; // default
...
async function main() {
	...
	if (cmd === 'convert') {
		...
		if (!res.text) throw new Error('No QR code found in input');
		const layers = Number(args.layers ?? 3);
		const colours = parseColourList(normaliseColoursArg(args.colours) as string[] | undefined);
	...
	if (cmd === 'generate') {
		...
		const layers = Number(args.layers ?? 3);
		const colours = parseColourList(normaliseColoursArg(args.colours) as string[] | undefined);
		const mode = (args.mode as string) ?? 'square';
		const composition = (args.composition as string) ?? 'discrete';
		const ecc = (args.ecc as string) ?? 'M';
		const out = String(args.out ?? 'qr.svg');
		const format = (args.format as string) ?? (out.toLowerCase().endsWith('.png') ? 'png' : out.toLowerCase().endsWith('.jpg') || out.toLowerCase().endsWith('.jpeg') ? 'jpeg' : 'svg');
		...
		const layering = (args.layering as string) ?? 'split';
		const finderKeyStyle = (args.finderKeyStyle as string) ?? 'finderFill';
		...
		const { svg } = await generateColourQr(data, {
			...
			modulePx,
			marginModules,
			eccLevel: (ecc === 'L' || ecc === 'M' || ecc === 'Q' || ecc === 'H') ? (ecc as any) : 'M',
	...
	if (cmd === 'decode') {
        ...
        if (process.env.SPQR_DEBUG) console.log('=== Trying SPQR two-layer decode ===');
        ...
        if (process.env.SPQR_DEBUG) console.log('=== SPQR two-layer decode failed, trying integrated color-aware decode ===');
