...
function majorityFilterMods(mods: boolean[][]) {
	const h = mods.length, w = mods[0]?.length ?? 0;
	const out: boolean[][] = Array.from({ length: h }, () => Array<boolean>(w).fill(false));
	for (let y = 0; y < h; y++) {
		for (let x = 0; x < w; x++) {
			let dark = 0, tot = 0;
			for (let dy = -1; dy <= 1; dy++) {
				for (let dx = -1; dx <= 1; dx++) {
					...
					if (yy >= 0 && yy < h && xx >= 0 && xx < w) { tot++; dark += mods[yy][xx] ? 1 : 0; }
				}
			}
			out[y][x] = dark >= Math.ceil(tot / 2);
		}
	}
	for (let y = 0; y < h; y++) for (let x = 0; x < w; x++) mods[y][x] = out[y][x];
}

function enforceFunctionPatterns(mods: boolean[][]) {
	const n = mods.length;
...
function parseSvgMeta(svg: string): { w?: number; h?: number; modulesW?: number; modulesH?: number; margin?: number; modulePx?: number; colours?: string[] } {
	const num = (re: RegExp) => {
...
function renderGridToRgba(modW: number, modH: number, margin: number, modulePx: number, blackSet: Set<string>): { data: Uint8ClampedArray; width: number; height: number } {
...
async function decodeSvgViaRects(inputPath: string): Promise<DecodeResult | null> {
	const svg = await readFile(inputPath, 'utf8');
	const meta = parseSvgMeta(svg);
	if (!meta.modulesW || !meta.modulesH || meta.margin === undefined || !meta.modulePx) return null;
	...
	while ((m = rectRe.exec(svg)) !== null) {
		...
		const w = Number(m[3]);
		const h = Number(m[4]);
		if (w !== meta.modulePx || h !== meta.modulePx) continue; // only 1x1 module rects
		// Convert pixel coords back to module grid
		const gridX = x / meta.modulePx - meta.margin;
		const gridY = y / meta.modulePx - meta.margin;
		if (gridX !== Math.floor(gridX) || gridY !== Math.floor(gridY)) continue;
		if (gridX < 0 || gridY < 0 || gridX >= meta.modulesW || gridY >= meta.modulesH) continue;
	...
	if (!code) return { text: null, format: 'none' };
	return { text: code.data, format: 'qr' };
}

export async function decodeImageToText(inputPath: string): Promise<DecodeResult> {
	...
	if (inputPath.toLowerCase().endsWith('.svg')) {
		try {
			...
			const w = meta.w ?? 512;
			const h = meta.h ?? 512;
			image = sharp(Buffer.from(svg)).resize(w, h, { kernel: 'nearest' as any });
		} catch {
			// fallback silently
		}
		// For SVGs, upscale to make modules crisp for jsQR
		image = image.resize({ width: 1024, height: 1024, fit: 'inside', kernel: 'nearest' as any });
	}
	...
	if (inputPath.toLowerCase().endsWith('.svg')) {
		try {
			...
			if (meta.modulesW && meta.modulesH && meta.margin !== undefined && meta.modulePx) {
				const fx = { tl: 3, tr: meta.modulesW - 4, bl: 3 } as const;
				const fy = { tl: 3, tr: 3, bl: meta.modulesH - 4 } as const;
				const coords = [
					{ x: fx.tl, y: fy.tl },
					{ x: fx.tr, y: fy.tr },
					{ x: fx.bl, y: fy.bl }
				...
				for (const c of coords) {
					const topLeftX: number = (c.x - 1 + (meta.margin as number)) * (meta.modulePx as number);
					const topLeftY: number = (c.y - 1 + (meta.margin as number)) * (meta.modulePx as number);
					const size: number = 3 * (meta.modulePx as number);
					for (let yy: number = topLeftY; yy < topLeftY + size; yy++) {
						for (let xx: number = topLeftX; xx < topLeftX + size; xx++) {
	...
	for (let i = 0; i < data.length; i += 4) {
		...
		mono[i] = v; mono[i + 1] = v; mono[i + 2] = v; mono[i + 3] = 255;
	}
...
export async function decodeRasterTwoLayer(inputPath: string): Promise<{ base: string | null; red: string | null; combined: string | null; }> {
	let image = sharp(inputPath);
	if (inputPath.toLowerCase().endsWith('.svg')) {
		const svg = await readFile(inputPath, 'utf8');
	...
	for (let i = 0, p = 0; p < pixels; i += 4, p++) {
		...
		const sum = r + g + b;
		if (sum < 50) maskBlack[p] = 1; // Super strict black threshold
	}
	// Estimate module size by run-lengths on center row/col
	function estimateModulePx(mask: Uint8Array): number {
		const rowY = Math.floor(height / 2);
		let runs: number[] = [];
		let curr = 0; let last = 0;
		for (let x = 0; x < width; x++) {
			const v = mask[rowY * width + x];
			if (v === last) curr++; else { if (curr > 2 && curr < 128) runs.push(curr); last = v; curr = 1; }
		}
		if (curr > 2 && curr < 128) runs.push(curr);
		if (runs.length === 0) return 8;
		// GCD of small runs
	...
	function sampleMean(x0: number, y0: number, x1: number, y1: number): { r: number; g: number; b: number } {
		...
		for (let y = minY; y <= maxY; y++) {
			for (let x = minX; x <= maxX; x++) {
				const p = (y * width + x) * 4;
				rs += data[p]; gs += data[p + 1]; bs += data[p + 2]; c++;
			}
		}
	...
	const isRed = (c: {r:number;g:number;b:number}) => c.r > c.g + 30 && c.r > c.b + 30 && c.r > 100;
	const isGreen = (c: {r:number;g:number;b:number}) => c.g > c.r + 30 && c.g > c.b + 30 && c.g > 100;
	const isBlack = (c: {r:number;g:number;b:number}) => c.r + c.g + c.b < 120;
	const isWhite = (c: {r:number;g:number;b:number}) => c.r > 200 && c.g > 200 && c.b > 200;
	...
	const len = (v:{r:number;g:number;b:number}) => Math.max(1e-6, Math.hypot(v.r, v.g, v.b));
	...
	const dot = (a:{r:number;g:number;b:number}, b:{r:number;g:number;b:number}) => a.r*b.r + a.g*b.g + a.b*b.b;

	for (let i = 0, p = 0; p < pixels; i += 4, p++) {
		const r = data[i], g = data[i + 1], b = data[i + 2];
		const l = len({ r, g, b }); const nv = { r: r / l, g: g / l, b: b / l };
		const sum = r + g + b;
		
		// Black detection (conservative)
		if (sum < 140 || dot(nv, nBlack) > 0.985) maskBlack[p] = 1;
		
		if (is4ColorScheme) {
			// 4-color BWRG scheme: strict red/green detection
			if ((dot(nv, nRed) > 0.94 && r > 110) || (r > g + 25 && r > b + 25 && r > 120)) maskRed[p] = 1;
			if ((dot(nv, nGreen) > 0.94 && g > 110) || (g > r + 25 && g > b + 25 && g > 120)) maskGreen[p] = 1;
		} else {
			// 8-color CMYRGB scheme: broader detection for multi-hue colors
			// Red detection (includes magenta, yellow)
			if (r > 120 && (r > g + 15 || r > b + 15)) maskRed[p] = 1;
			// Green detection (includes cyan, yellow)  
			if (g > 120 && (g > r + 15 || g > b + 15)) maskGreen[p] = 1;
		}
	}
	...
	for (let p = 0; p < pixels; p++) {
		if (maskBlack[p]) blackCount++;
		if (maskRed[p]) redCount++;
		if (maskGreen[p]) greenCount++;
	}
	...
	function addSyntheticFinder(grid: number[][], ox: number, oy: number) {
		for (let dy = 0; dy < 7; dy++) {
			for (let dx = 0; dx < 7; dx++) {
	...
	for (let gy = 0; gy < gridH; gy++) {
		for (let gx = 0; gx < gridW; gx++) {
			if (layerAGrid[gy][gx]) layerACount++;
			if (layerBGrid[gy][gx]) layerBCount++;
		}
	}
			...
			for (let gx = 0; gx < gridW; gx++) {
				...
				for (let py = py0; py < py1; py++) {
					for (let px = px0; px < px1; px++) {
						...
						img[idx] = value; img[idx + 1] = value; img[idx + 2] = value; img[idx + 3] = 255;
					}
				}
			}
		}
	...
	for (let i = 0, p = 0; p < pixels; i += 4, p++) {
		...
		blackOnlyImage[i] = value; blackOnlyImage[i + 1] = value; blackOnlyImage[i + 2] = value; blackOnlyImage[i + 3] = 255;
	}
    ...
    if (DEBUG) {
        ...
        } catch (e) { console.log(`Failed to save debug image:`, e); }
    }
    ...
    if (DEBUG) {
        ...
        } catch (e) { console.log('Failed to save layer A debug image'); }
    }
	
	const sliceToMods = (grid: number[][]): boolean[][] => Array.from({ length: modulesH }, (_, my) =>
		Array.from({ length: modulesW }, (_, mx) => grid[margin + my][margin + mx] === 1)
	);
	const decodeWithInternal = (mods: boolean[][]): string | null => {
    ...
    if (DEBUG) {
        ...
        } catch (e) { console.log('Failed to save layer B debug image'); }
    }
    ...
    if (DEBUG) {
        const saveDebugImage = async (imgData: Uint8ClampedArray, name: string) => {
            try {
                const buf = Buffer.from(imgData);
                await sharp(buf, { raw: { width, height, channels: 4 } }).png().toFile(`out/debug_${name}.png`);
            } catch (e) { console.log(`Failed to save debug image ${name}:`, e); }
        };
	...
	if (base && red) combined = `${base}${red}`; else combined = base;
	return { base, red, combined };
}

function addStandardFinders(set: Set<string>, w: number, h: number) {
	const drawFinder = (ox: number, oy: number) => {
		for (let dy = 0; dy < 7; dy++) {
			for (let dx = 0; dx < 7; dx++) {
				...
				const inOuter = dx === 0 || dx === 6 || dy === 0 || dy === 6;
				const inInner = dx >= 2 && dx <= 4 && dy >= 2 && dy <= 4;
				if (inOuter || inInner) set.add(`${x},${y}`);
			}
		}
	};
	drawFinder(0, 0);
...
export async function decodeSvgMultiLayer(inputPath: string): Promise<{ base: string | null; red: string | null; green: string | null; combined: string | null; twoLayerA?: string | null; twoLayerB?: string | null; }> {
	const svg = await readFile(inputPath, 'utf8');
	const meta = parseSvgMeta(svg);
	if (!meta.modulesW || !meta.modulesH || meta.margin === undefined || !meta.modulePx) {
	...
	if (embedded) {
		try {
			...
			const cols = (json.colours as string[]) || ['#000000','#ff0000','#00ff00'];
			const modW = json.width as number; const modH = json.height as number;
			const margin = json.margin as number; const modulePx = Math.max(8, json.modulePx as number);
			const toSet = (idx: number): Set<string> => {
				if (!json.matrices?.[idx]?.bits) return new Set<string>();
				const bits: string = json.matrices[idx].bits;
				const s = new Set<string>();
				for (let y = 0; y < modH; y++) {
					for (let x = 0; x < modW; x++) {
						const i = y * modW + x; if (bits[i] === '1') s.add(`${x},${y}`);
					}
				}
				return s;
			};
			...
			const redWithFinders = new Set<string>(redSet); addStandardFinders(redWithFinders, modW, modH);
			...
			const greenWithFinders = new Set<string>(greenSet); addStandardFinders(greenWithFinders, modW, modH);
			raster = renderGridToRgba(modW, modH, margin, modulePx, greenWithFinders);
			out = jsQR(raster.data, raster.width, raster.height);
			const green = out ? out.data : null;
			let combined: string | null = null;
			if (base && red && green) combined = `${base}${red}${green}`; else if (base && red) combined = `${base}${red}`; else combined = base;
			// Two-layer downmix: A = base ∪ green, B = red ∪ green
			const twoASet = new Set<string>([...baseSet, ...greenSet]);
			const twoBSet = new Set<string>([...redSet, ...greenSet]);
			const twoAFinders = new Set<string>(twoASet); addStandardFinders(twoAFinders, modW, modH);
			const twoBFinders = new Set<string>(twoBSet); addStandardFinders(twoBFinders, modW, modH);
	...
	const modW = meta.modulesW; const modH = meta.modulesH;
	const rectModulePx = meta.modulePx as number;
	const modulePx = Math.max(8, rectModulePx);
	// Helper to collect rects of a given colour with 1x1 module size
	const collect = (hex: string): Set<string> => {
		...
		while ((m = re.exec(svg)) !== null) {
			...
			const gx = x / rectModulePx - (meta.margin as number);
			const gy = y / rectModulePx - (meta.margin as number);
			if (gx >= 0 && gy >= 0 && gx < modW && gy < modH && Number.isInteger(gx) && Number.isInteger(gy)) s.add(`${gx},${gy}`);
		}
		return s;
	};
	// Detect key colours from finder inner 3x3 blocks if present
	const total = (meta.modulePx as number) * 3;
	const pos = (fx: number, fy: number) => ({ x: (fx - 1 + (meta.margin as number)) * (meta.modulePx as number), y: (fy - 1 + (meta.margin as number)) * (meta.modulePx as number) });
	const tl = pos(3, 3); const tr = pos((meta.modulesW as number) - 4, 3); const bl = pos(3, (meta.modulesH as number) - 4);
	...
	const redWithFinders = new Set<string>(redSet);
	...
	const greenWithFinders = new Set<string>(greenSet);
	addStandardFinders(greenWithFinders, modW, modH);
	raster = renderGridToRgba(modW, modH, meta.margin, modulePx, greenWithFinders);
	out = jsQR(raster.data, raster.width, raster.height);
	const green = out ? out.data : null;
	let combined: string | null = null;
	if (base && red && green) combined = `${base}${red}${green}`;
	else if (base && red) combined = `${base}${red}`;
	else combined = base;
